  ...
  async function load() {
    ...
    try {
      ...
      const gData   = {
        ...
        maxSize: Number(g.maxSize), borrower: g.borrower, members,
      };
      setGroup(gData);

      // Get vote round data from event logs
      // voteRound is a struct — read it via events or direct call
      // We check hasVoted for current user
      const voted = await contract.hasVoted(gid, account);
      if (voted) {
        // find who they voted for via VoteCast events
        // ONLY showing changed parts (full logic preserved)
...
const startEvents = await contract.queryFilter(
  contract.filters.VotingStarted(),
  latestBlock - 5000,
  latestBlock
);

const resolveEvents = await contract.queryFilter(
  contract.filters.BorrowerSelected(),
  latestBlock - 5000,
  latestBlock
);
        const myEvent    = voteEvents.find(e =>
          Number(e.args[0]) === gid &&
          e.args[1].toLowerCase() === account.toLowerCase()
        );
        if (myEvent) setMyVote(myEvent.args[2]);
      }

      // Get vote counts for all members
      const counts = {};
      for (const m of members) {
        const c = await contract.votesReceived(gid, m);
        counts[m] = Number(c);
      }
      setVoteCounts(counts);

      // Detect vote state from events
      const startEvents    = await contract.queryFilter("VotingStarted");
      const resolveEvents  = await contract.queryFilter("BorrowerSelected");
      const myStartEvent   = startEvents.filter(e => Number(e.args[0]) === gid).pop();
      const myResolveEvent = resolveEvents.filter(e => Number(e.args[0]) === gid).pop();
      ...
      if (myResolveEvent) {
        ...
        setVoteEnd(Number(myStartEvent.args[1]));
        setVoteState(Date.now() / 1000 > Number(myStartEvent.args[1]) ? 2 : 1);
      } else {
        setVoteState(0);
      }

    } catch (err) {
      console.error("VotingScreen load:", err);
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading) return <div className="empty-state"><div className="empty-icon">⟳</div><p>Loading voting data...</p></div>;
  ...
  return (
    <div>
      ...
      {voteState === 1 && (
        <div style={{ background: "var(--purple-dim)", border: "1px solid var(--purple-mid)", borderRadius: "var(--radius)", padding: "14px 18px", marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ color: "var(--purple)", fontWeight: 600, marginBottom: 2 }}>🟢 VOTING IS OPEN</div>
            <div style={{ color: "var(--text2)", fontSize: 13 }}>{totalVotes} of {group?.members?.length} votes cast</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: timeLeft === "Closed" ? "var(--red)" : "var(--amber)" }}>{timeLeft || "..."}</div>
      ...
      {voteState === 2 && winner && (
        <div style={{ background: "var(--green-dim)", border: "1px solid rgba(34,197,94,.3)", borderRadius: "var(--radius)", padding: "18px 20px", marginBottom: 20 }}>
          ...
          <div style={{ color: "var(--text2)", fontSize: 13 }}>Selected borrower:</div>
          <div style={{ fontFamily: "monospace", fontSize: 15, color: "var(--text)", fontWeight: 600, marginTop: 4 }}>
            {winner}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div className="card">
          <div className="card-header">
            ...
            {voteState === 0 && group?.members?.length === group?.maxSize && (
              <button className="btn-primary" style={{ padding: "6px 16px", fontSize: 12 }} onClick={handleStartVoting} disabled={txPending}>
                Start Voting
              </button>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {group?.members?.map((m, i) => {
              ...
              return (
                <div key={i} style={{ background: "var(--bg3)", borderRadius: "var(--radius-sm)", padding: "12px 14px", border: isWinner ? "1px solid var(--green)" : "1px solid var(--border)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <div>
                      <span style={{ fontFamily: "monospace", fontSize: 12, color: isMe ? "var(--cyan)" : "var(--text)" }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 20, fontWeight: 700, color: "var(--cyan)" }}>{votes}</div>
                  ...
                  <div className="vote-bar-bg" style={{ marginBottom: 8 }}>
                    <div className="vote-bar-fill" style={{ width: `${pct}%`, background: isWinner ? "var(--green)" : "var(--purple)" }} />
                  </div>
        ...
        <div>
          ...
          <div className="card" style={{ marginBottom: 16 }}>
            <div className="card-title" style={{ marginBottom: 14 }}>Vote Summary</div>
            {[
          ...
          {voteState === 1 && (timeLeft === "Closed" || totalVotes === group?.members?.length) && (
            <div className="card">
              <div className="card-title" style={{ marginBottom: 10 }}>Resolve Vote</div>
              <p style={{ fontSize: 13, color: "var(--text2)", marginBottom: 14 }}>
                {timeLeft === "Closed" ? "Voting window has closed." : "All members have voted."}
                {" "}Click below to count votes and select the borrower.
              </p>
              <button className="btn-primary" style={{ width: "100%" }} onClick={handleResolve} disabled={txPending}>
